Shader "Quiz cAnners/TextMeshPro/Distance Field" {
...
SubShader {
	...
	Pass {
		...
		#pragma shader_feature_local _MODE_FILLSCREEN  _MODE_PIXPERFECT 
		...
		pixel_t VertShader(vertex_t input)
		{
...
#if qc_USE_PARALLAX
			_UnderlayOffsetX = min(1, max(-1, -qc_ParallaxOffset.x * 16));
			_UnderlayOffsetY = min(1, max(-1, -qc_ParallaxOffset.y * 16));
#endif
			...
			float texAspect = _OutlineTex_TexelSize.y * _OutlineTex_TexelSize.z;
			...
			output.stretch = aspectCorrection;
#endif
...
#if _MODE_FILLSCREEN
			float2 fragCoord = (screenUv - 0.5) * input.stretch.xy + 0.5;
#else
			float2 fragCoord = screenUv * _ScreenParams.xy * _OutlineTex_TexelSize.xy;
#endif

			// Screen space UV here
		
			//input.textures.zw + float2(_OutlineUVSpeedX, _OutlineUVSpeedY) * _Time.y
			outlineColor *=  tex2D(_OutlineTex, fragCoord);
