Shader "Quiz cAnners/Effects/Thin Lines/Hexagon" {
	Properties{
			...
			Pass{
				...
				float4 frag(v2f i) : COLOR
				{
					...
					*/
					...
					float2 sampleTex = float2(pow(0.5 - dist, 6) + inner * 0.25, angle);

					float2 animOffset = 0.2 * float2(_Time.x, -_Time.x);

					float4 tex =
						tex2D(_MainTex, sampleTex + animOffset * 3.82)
						* 
						tex2D(_MainTex, sampleTex - animOffset*2.75)
						;

					//float circle = smoothstep(0.999, 20, brightness);

				

					float masking = 0.25 + smoothstep(0.05, 0.5, tex.r * brightness)*2;
